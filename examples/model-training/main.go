// Command model-training runs group-relative model training as a Workload.
// The graph uses these data-flow properties:
//
...
//
// The learner writes checkpoints to shared storage and broadcasts references.
// Each rollout replica loads a reference before acknowledging its epoch.
//
...
func main() {
	fs := flag.NewFlagSet("model-training", flag.ExitOnError)
	...
	if len(os.Args) < 2 {
		log.Fatal("usage: model-training [flags] prompts|rollout|reward|advantage|learner|score|dump")
	}
	...
	// out here than after an expensive training run.
	if op == "score" {
...
// sidecar binds its port only after backend initialization, so a
